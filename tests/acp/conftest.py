...
def _pythinker_bin() -> str:
    """Return the path to the pythinker entry-point script inside the venv."""
    return str(_repo_root() / ".venv" / "bin" / "pythinker")
...
def acp_share_dir(tmp_path: Path) -> Path:
    ...
    trace_env = os.getenv("PYTHINKER_SCRIPTED_ECHO_TRACE", "0")
    config_data = {
        ...
        "providers": {
            "scripted_provider": {
                ...
                "api_key": "",
                "env": {
                    "PYTHINKER_SCRIPTED_ECHO_SCRIPTS": str(scripts_path),
                    "PYTHINKER_SCRIPTED_ECHO_TRACE": trace_env,
                },
            }
        },
    }
    ...
    (credentials_dir / "pythinker-code.json").write_text(
        json.dumps(
            {
...
async def acp_client(
    ...
    """Spawn a pythinker ACP subprocess and return the SDK connection + test client."""
    test_client = ACPTestClient()
    env = {
        **os.environ,
        "PYTHINKER_SHARE_DIR": str(acp_share_dir),
    }
