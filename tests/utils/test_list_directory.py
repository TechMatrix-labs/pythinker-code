...
from pythinker_code.utils.path import _LIST_DIR_CHILD_WIDTH, _LIST_DIR_ROOT_WIDTH, list_directory


@pytest.mark.skipif(platform.system() == "Windows", reason="Unix-specific symlink tests.")
async def test_list_directory_tree_unix(temp_work_dir: HostPath) -> None:
...
async def test_list_directory_truncates_root_width(temp_work_dir: HostPath) -> None:
...
async def test_list_directory_truncates_child_width(temp_work_dir: HostPath) -> None:
...
async def test_list_directory_dirs_before_files(temp_work_dir: HostPath) -> None:
...
async def test_list_directory_empty(temp_work_dir: HostPath) -> None:
...
async def test_list_directory_unreadable_subdir(temp_work_dir: HostPath) -> None:
...
async def test_list_directory_last_entry_is_dir(temp_work_dir: HostPath) -> None:
...
async def test_list_directory_tree_windows(temp_work_dir: HostPath) -> None:
    await (temp_work_dir / "regular.txt").write_text("hello")
    await (temp_work_dir / "adir").mkdir()
    await (temp_work_dir / "adir" / "inside.txt").write_text("world")
    await (temp_work_dir / "emptydir").mkdir()
