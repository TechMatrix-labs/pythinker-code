...
from pythinker_code.utils.slashcmd import (
    SlashCommand,
    SlashCommandCall,
...
def test_parse_slash_command_call():
    ...
    # Edge cases: non-ASCII characters in args (should work)
    result = parse_slash_command_call("/echo café résumé")
    assert result == snapshot(
        SlashCommandCall(name="echo", args="café résumé", raw_input="/echo café résumé")
    )

    result = parse_slash_command_call("/search naïve query english query")
    assert result == snapshot(
        SlashCommandCall(
            name="search",
            args="naïve query english query",
            raw_input="/search naïve query english query",
        )
    )

    result = parse_slash_command_call("/skill:update-docs some args   with spaces")
    assert result == snapshot(
        SlashCommandCall(
            name="skill:update-docs",
            args="some args   with spaces",
            raw_input="/skill:update-docs some args   with spaces",
        )
    )

    # Non-ASCII characters in command name should fail (regex only allows a-zA-Z0-9_- and :)
    assert parse_slash_command_call("/café args") is None
    assert parse_slash_command_call("/naïve") is None
