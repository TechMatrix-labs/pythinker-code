...
def _spawn_workers(
    ...
    *,
    ...
    env = os.environ.copy()
    env["PYTHINKER_SHARE_DIR"] = str(share)
...
def test_cross_process_lock_mutual_exclusion(tmp_path: Path) -> None:
    ...
    script = textwrap.dedent(f"""\
        import asyncio, sys
        from pathlib import Path
        from pythinker_code.auth.oauth import _CrossProcessLock
...
def test_atomic_save_no_corruption(tmp_path: Path) -> None:
    ...
    script = textwrap.dedent(f"""\
        ...
        from pathlib import Path
        from pythinker_code.auth.oauth import OAuthToken, _save_to_file
    ...
    cred = tmp_path / "share" / "credentials" / "pythinker-code.json"
...
async def test_lock_file_created_with_safe_permissions(tmp_path: Path) -> None:
    ...
    share = tmp_path / "share"
    (share / "credentials").mkdir(parents=True, exist_ok=True)
    original = os.environ.get("PYTHINKER_SHARE_DIR")
    os.environ["PYTHINKER_SHARE_DIR"] = str(share)
    try:
        lock = _CrossProcessLock("oauth/pythinker-code")
        acquired = await lock.acquire_with_retry()
        assert acquired
        lock_path = share / "credentials" / "pythinker-code.lock"
        ...
        if original is None:
            os.environ.pop("PYTHINKER_SHARE_DIR", None)
        else:
            os.environ["PYTHINKER_SHARE_DIR"] = original
