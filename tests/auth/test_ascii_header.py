...
from pythinker_code.auth.oauth import _ascii_header_value, _common_headers
...
class TestAsciiHeaderValue:
    ...
    def test_all_non_ascii_returns_fallback(self) -> None:
        assert _ascii_header_value("éé") == "unknown"


class TestCommonHeaders:
    """Test that _common_headers returns clean header values."""

    @patch("pythinker_code.auth.oauth.platform")
    @patch("pythinker_code.auth.oauth.get_device_id", return_value="abc123")
