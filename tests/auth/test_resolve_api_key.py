...
def _make_config(*, with_oauth: bool = True, api_key: str = "") -> Config:
    provider = LLMProvider(
        ...
        oauth=OAuthRef(storage="file", key="oauth/pythinker-code") if with_oauth else None,
    )
    model = LLMModel(
...
def _make_oauth_manager(config: Config, initial_token: OAuthToken | None = None) -> OAuthManager:
    ...
    with patch("pythinker_code.auth.oauth.load_tokens", return_value=initial_token):
...
def test_resolve_api_key_returns_oauth_token_when_available():
    ...
    ref = OAuthRef(storage="file", key="oauth/pythinker-code")
...
def test_resolve_api_key_falls_back_to_api_key_when_no_token():
    ...
    with patch("pythinker_code.auth.oauth.load_tokens", return_value=None):
...
def test_get_chatgpt_account_id_returns_persisted_account_id():
    config = _make_config(with_oauth=True)
    token = OAuthToken(
        access_token="oauth-access-123",
        refresh_token="refresh-123",
    ...
    oauth = _make_oauth_manager(config, initial_token=token)
    ref = OAuthRef(storage="file", key="oauth/openai-chatgpt")

    with patch("pythinker_code.auth.oauth.load_tokens", return_value=token):
        result = oauth.get_chatgpt_account_id(ref)

    assert result == "account-123"


@pytest.mark.asyncio
async def test_refresh_preserves_chatgpt_account_id(tmp_path, monkeypatch):
    monkeypatch.setenv("PYTHINKER_SHARE_DIR", str(tmp_path))
    config = _make_config(with_oauth=True)
    ref = OAuthRef(storage="file", key="oauth/openai-chatgpt")
    config.providers["managed:pythinker-code"].oauth = ref
    token = OAuthToken(
    ...
    with patch.object(OAuthManager, "_refresh_token_for_ref", AsyncMock(return_value=refreshed)):
        await oauth.ensure_fresh(force=True)

    restored = load_tokens(ref)
    assert restored is not None
    assert restored.account_id == "account-123"
...
def test_resolve_api_key_falls_back_when_token_has_empty_access_token():
    ...
    with patch("pythinker_code.auth.oauth.load_tokens", return_value=empty_token):
...
async def test_resolve_api_key_falls_back_after_rejected_refresh_token(tmp_path, monkeypatch):
    ...
    result = oauth.resolve_api_key(config.providers["managed:pythinker-code"].api_key, ref)
    assert result == "fallback-key"
...
async def test_ensure_fresh_without_runtime_caches_token():
    ...
    with patch("pythinker_code.auth.oauth.load_tokens", return_value=fresh_token):
...
async def test_ensure_fresh_without_runtime_refreshes_expired_token():
    ...
    with (
        patch("pythinker_code.auth.oauth.load_tokens", return_value=expired_token),
        patch(
            "pythinker_code.auth.oauth.refresh_token",
