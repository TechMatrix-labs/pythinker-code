...
from pythinker_code.ui.shell.placeholders import PromptPlaceholderManager
...
async def test_open_in_external_editor_uses_provider_value(monkeypatch) -> None:
    ...
    monkeypatch.setattr("pythinker_code.utils.editor.get_editor_command", fake_get_editor_command)
    monkeypatch.setattr("pythinker_code.utils.editor.edit_text_in_editor", fake_edit_text_in_editor)
...
async def test_open_in_external_editor_expands_and_refolds_text_placeholders(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "pythinker_code.utils.editor.get_editor_command", lambda configured=None: ["vim"]
    )
...
def test_open_in_external_editor_toast_when_no_editor(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "pythinker_code.utils.editor.get_editor_command", lambda configured=None: None
    )
