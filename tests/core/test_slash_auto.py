"""Tests for /auto and /yolo slash command independence."""
...
from pythinker_code.soul.slash import auto as auto_slash
from pythinker_code.soul.slash import yolo as yolo_slash
...
def _make_soul(runtime: Runtime, tmp_path: Path) -> PythinkerSoul:
    ...
    runtime.approval.set_auto(False)
    agent = Agent(
        name="Test Agent",
...
async def _run(fn, soul: PythinkerSoul, args: str = "") -> None:
    result = fn(soul, args)
    if result is not None:
        await result


async def test_auto_slash_toggles_auto_flag(
    ...
    assert soul.runtime.approval.is_auto() is False
    await _run(auto_slash, soul)
    assert soul.runtime.approval.is_auto() is True
    assert soul.runtime.approval.is_auto_flag() is True
    assert any("auto" in s.text.lower() and "enabled" in s.text.lower() for s in sent)

    # Toggle off.
    sent.clear()
    await _run(auto_slash, soul)
    assert soul.runtime.approval.is_auto() is False
    assert soul.runtime.approval.is_auto_flag() is False
    assert any("auto" in s.text.lower() and "disabled" in s.text.lower() for s in sent)


async def test_auto_slash_notifies_injection_providers(
    runtime: Runtime, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
async def test_auto_slash_does_not_touch_yolo_flag(
    runtime: Runtime, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    soul = _make_soul(runtime, tmp_path)
...
async def test_auto_slash_off_appends_context_reminder(
    runtime: Runtime, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    soul = _make_soul(runtime, tmp_path)
    ...
    assert "Auto mode is now disabled" in reminder
    assert "Ignore any earlier auto mode reminders" in reminder
    assert "AskUserQuestion is available again" in reminder


async def test_auto_slash_off_clears_runtime_auto_overlay(
    runtime: Runtime, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    soul = _make_soul(runtime, tmp_path)
    soul.runtime.approval.set_runtime_auto(True)
    assert soul.runtime.approval.is_auto() is True
    assert soul.runtime.approval.is_auto_flag() is False
    monkeypatch.setattr("pythinker_code.soul.slash.wire_send", lambda _msg: None)
    ...
    assert soul.runtime.approval.is_auto() is False
    assert soul.runtime.approval.is_runtime_auto() is False


async def test_yolo_slash_under_auto_only_toggles_yolo_flag(
    runtime: Runtime, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Regression: /yolo under auto-only used to fire the 'require approval' branch
    and claim approval is now required, while auto kept auto-approving.
    /yolo must inspect only the yolo flag, not the auto-approve state."""
    soul = _make_soul(runtime, tmp_path)
    # Auto on, yolo flag off -> auto-approve True, but yolo flag False.
    soul.runtime.approval.set_auto(True)
...
async def test_yolo_slash_off_under_auto_does_not_claim_approval_required(
    runtime: Runtime, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Regression: /yolo off while auto is on must not claim 'approval required'.

    Prior bug: the toast emitted "Actions will require approval" while auto kept
    tool calls auto-approved. The toast now calls out that auto is still keeping
    auto-approve on.
    """
    soul = _make_soul(runtime, tmp_path)
    soul.runtime.approval.set_yolo(True)
    ...
    # Yolo flag flipped off; auto stays on; effective auto-approve stays on.
    assert soul.runtime.approval.is_yolo_flag() is False
    assert soul.runtime.approval.is_auto() is True
    assert soul.runtime.approval.is_yolo() is False
    assert soul.runtime.approval.is_auto_approve() is True
...
async def test_status_snapshot_separates_yolo_and_auto(runtime: Runtime, tmp_path: Path) -> None:
    """Status bar must tell yolo_enabled and auto_enabled apart.

    Previously `yolo_enabled` reflected the auto-approve state, which showed
    'yolo' in the status bar even when only auto was on. Now yolo_enabled only
    reflects the explicit yolo flag.
    """
    soul = _make_soul(runtime, tmp_path)

    # Auto only -> auto_enabled True, yolo_enabled False.
    soul.runtime.approval.set_auto(True)
    snap = soul.status
    assert snap.auto_enabled is True
