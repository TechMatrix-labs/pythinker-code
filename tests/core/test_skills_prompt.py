...
def _skill(name: str, scope: str, description: str = "desc") -> Skill:
    skill_dir = HostPath.unsafe_from_local_path(Path(f"/tmp/{scope}/{name}"))
    return Skill(
        name=name,
...
async def test_end_to_end_project_override_renders_correctly(tmp_path, monkeypatch):
    ...
    ("pythinker cli does not support proj scope skills") is fixed: when a
    project-local skill shares a name with skills at other scopes, the project
    version is the one injected into the system prompt, grouped under
    ``### Project``.
    """
