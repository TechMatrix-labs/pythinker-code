...
from pythinker_code.cli.plugin import _parse_git_url, _resolve_source
...
def test_parse_full_plugin_json(tmp_path: Path):
    plugin_dir = _write_plugin(
        tmp_path,
        {
            ...
            "inject": {"pythinkercode.api_key": "api_key"},
        },
    )
    spec = parse_plugin_json(plugin_dir / "plugin.json")
    assert spec.name == "stock-assistant"
    assert spec.config_file == "config/config.json"
    assert spec.inject == {"pythinkercode.api_key": "api_key"}
...
def test_parse_plugin_json_with_runtime(tmp_path: Path):
    plugin_dir = _write_plugin(
        tmp_path,
        {
            ...
            "version": "1.0.0",
            "runtime": {"host": "pythinker-code", "host_version": "1.22.0"},
        },
    )
    spec = parse_plugin_json(plugin_dir / "plugin.json")
    assert spec.runtime is not None
    assert spec.runtime.host == "pythinker-code"
...
def test_inject_config_writes_value(tmp_path: Path):
    ...
    (config_dir / "config.json").write_text(
        json.dumps({"pythinkercode": {"api_key": "PLACEHOLDER", "timeout": 30}}),
        encoding="utf-8",
    )
    ...
    assert result["pythinkercode"]["api_key"] == "sk-real-key"
    assert result["pythinkercode"]["timeout"] == 30  # untouched
...
def test_write_runtime(tmp_path: Path):
    ...
    runtime = PluginRuntime(host="pythinker-code", host_version="1.22.0")
    write_runtime(plugin_dir, runtime)

    data = json.loads((plugin_dir / "plugin.json").read_text())
    assert data["runtime"]["host"] == "pythinker-code"
