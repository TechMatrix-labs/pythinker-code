"""Tests for ``PythinkerCLI.shutdown_background_tasks``.
...
def _make_cli(
    *,
...
) -> tuple[PythinkerCLI, MagicMock, dict]:
    """Build a PythinkerCLI stub exposing only what shutdown_background_tasks needs.
    ...
    cli = PythinkerCLI.__new__(PythinkerCLI)
    cli._soul = MagicMock()
...
async def test_shutdown_prints_notice_and_kills_when_active(capsys) -> None:
    ...
    with patch("pythinker_code.app.asyncio.sleep", side_effect=fake_sleep):
...
async def test_shutdown_writes_to_original_stderr_when_redirected(monkeypatch) -> None:
    ...
    alone would silently send the notice into ``pythinker.log`` (where fd=2 now
    ...
    monkeypatch.setattr("pythinker_code.app.open_original_stderr", fake_open_original_stderr)
