"""E2E regression test for Pythinker-compatible endpoints that reject empty assistant text.
...
3. Some Pythinker-compatible gateways reject the second request if the prior assistant
...
class MockPythinkerCompatServer(Protocol):
    base_url: str
    requests: list[dict[str, Any]]


class _MockPythinkerCompatServer:
...
async def mock_pythinker_compat_server() -> AsyncIterator[MockPythinkerCompatServer]:
    server_holder: _MockPythinkerCompatServer | None = None
    ...
    server_holder = _MockPythinkerCompatServer(f"http://127.0.0.1:{port}")
...
def _write_pythinker_config(config_path: Path, *, base_url: str) -> None:
    config_path.write_text(
        json.dumps(
            {
                "default_model": "mock-pythinker",
                "models": {
                    "mock-pythinker": {
                        "provider": "mock-pythinker-provider",
                        "model": "pythinker-for-coding",
                        "max_context_size": 100000,
                    }
                },
                "providers": {
                    "mock-pythinker-provider": {
                        "type": "pythinker",
                        "base_url": base_url,
...
async def _run_pythinker_print_json(
    *,
    config_path: Path,
    share_dir: Path,
    work_dir: Path,
    ...
    env["PYTHINKER_DISABLE_TELEMETRY"] = "1"
    env["COLUMNS"] = "120"
    env["LINES"] = "40"
...
async def test_pythinker_compat_endpoint_accepts_tool_call_history_without_empty_content(
    tmp_path: Path, mock_pythinker_compat_server: MockPythinkerCompatServer
) -> None:
    ...
    _write_pythinker_config(config_path, base_url=f"{mock_pythinker_compat_server.base_url}/v1")

    return_code, stdout, stderr = await _run_pythinker_print_json(
    ...
    assert len(mock_pythinker_compat_server.requests) == 2
    assistant_message = _find_assistant_tool_call_message(mock_pythinker_compat_server.requests[1])
