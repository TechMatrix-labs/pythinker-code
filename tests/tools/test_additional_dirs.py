...
from pythinker_code.tools.file.glob import Params as GlobParams
...
@pytest.fixture
def additional_dir(temp_work_dir: HostPath) -> Generator[HostPath]:
...
@pytest.fixture
def runtime_with_additional_dir(runtime: Runtime, additional_dir: HostPath) -> Runtime:
...
async def test_glob_in_additional_dir(
    runtime_with_additional_dir: Runtime, additional_dir: HostPath
):
...
async def test_write_file_in_additional_dir(
    runtime_with_additional_dir: Runtime, approval: Approval, additional_dir: HostPath
):
...
async def test_add_dir_dynamically_affects_tools(runtime: Runtime, approval: Approval):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        extra = HostPath.unsafe_from_local_path(Path(tmpdir).resolve())
...
async def test_subagent_shares_additional_dirs(runtime: Runtime):
    ...
    runtime.additional_dirs.append(HostPath("/test/shared"))
    assert HostPath("/test/shared") in subagent_a.additional_dirs
    assert HostPath("/test/shared") in subagent_b.additional_dirs
...
@pytest.fixture
def skills_dir(temp_work_dir: HostPath) -> Generator[HostPath]:
...
@pytest.fixture
def runtime_with_skills_dir(runtime: Runtime, skills_dir: HostPath) -> Runtime:
    """Runtime with a skills directory configured."""
    runtime.skills_dirs.append(skills_dir)
    return runtime


async def test_glob_in_skills_dir(runtime_with_skills_dir: Runtime, skills_dir: HostPath):
...
async def test_glob_in_skills_dir_subdirectory(
    runtime_with_skills_dir: Runtime, skills_dir: HostPath
):
...
async def test_glob_skill_scripts_dir_outside_workspace(
    ...
    """
    with tempfile.TemporaryDirectory() as tmpdir:
        skills_root = HostPath.unsafe_from_local_path(Path(tmpdir).resolve())
...
async def test_subagent_shares_skills_dirs(runtime: Runtime):
    ...
    runtime.skills_dirs.append(HostPath("/fake/skills"))
