"""Tests for plan mode approval behavior under yolo and auto."""
...
from pythinker_code.tools.plan import ExitPlanMode, PlanOption
from pythinker_code.tools.plan import Params as ExitParams
from pythinker_code.tools.plan.enter import EnterPlanMode
from pythinker_code.tools.plan.enter import Params as EnterParams
...
async def test_exit_plan_auto_auto_approves(exit_tool: ExitPlanMode, tmp_path: Path):
    """Auto auto-approves plan approval because no user is present."""
    tracker: dict = {}
    plan_file = tmp_path / "plan.md"
    plan_file.write_text("# Test Plan\n- Step 1\n- Step 2")
...
async def test_exit_plan_auto_auto_approve_with_options(exit_tool: ExitPlanMode, tmp_path: Path):
    """Auto auto-approval works even when options are provided."""
    tracker: dict = {}
    plan_file = tmp_path / "plan.md"
