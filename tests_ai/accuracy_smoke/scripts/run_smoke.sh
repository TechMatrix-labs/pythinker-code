...
WHEEL_DIR="${PYTHINKER_CLI_WHEEL_DIR:-${REPO_ROOT}/dist/accuracy_smoke}"
JOBS_DIR="${HARBOR_JOBS_DIR:-${REPO_ROOT}/jobs}"
MODEL="${HARBOR_MODEL:-pythinker/pythinker-for-coding}"
AGENT_IMPORT_PATH="${HARBOR_AGENT_IMPORT_PATH:-tests_ai.accuracy_smoke.local_pythinker_code_agent:LocalPythinkerCli}"
...
if [ -z "${PYTHINKER_API_KEY:-}" ]; then
  echo "Missing API key. Set PYTHINKER_API_KEY first." >&2
  echo 'Example: export PYTHINKER_API_KEY="your_api_key"' >&2
  exit 1
fi

mkdir -p "${WHEEL_DIR}"
echo "Building local pythinker-code wheel into ${WHEEL_DIR}"
...
UV_PYTHON="${UV_PYTHON}" UV_PYTHON_INSTALL_MIRROR="${UV_PYTHON_INSTALL_MIRROR}" \
  uv build --package pythinker-code --out-dir "${WHEEL_DIR}" >/dev/null
PYTHINKER_CLI_WHEEL_PATH="$(ls -t "${WHEEL_DIR}"/*.whl | head -n 1)"

echo "Using local pythinker-code wheel: ${PYTHINKER_CLI_WHEEL_PATH}"
...
while IFS= read -r task || [ -n "${task}" ]; do
  ...
  PYTHINKER_CLI_WHEEL_PATH="${PYTHINKER_CLI_WHEEL_PATH}" \
