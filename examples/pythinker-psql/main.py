"""
pythinker-psql: AI-assisted PostgreSQL interactive terminal.
...
from pythinker_code.soul import LLMNotSet, LLMNotSupported, MaxStepsReached, RunCancelled, run_soul
from pythinker_code.soul.agent import Runtime
from pythinker_code.soul.context import Context
from pythinker_code.soul.pythinkersoul import PythinkerSoul
from pythinker_code.ui.shell.visualize import visualize
...
async def create_psql_soul(llm: LLM | None, conninfo: str) -> PythinkerSoul:
    """Create a PythinkerSoul configured for PostgreSQL with ExecuteSql tool
    and standard pythinker-code tools."""
    ...
    config = load_config()
    host_work_dir = HostPath.cwd()
    session = await Session.create(host_work_dir)
    ...
    cast(PythinkerToolset, agent.toolset).add(ExecuteSql(conninfo))
...
class PsqlShell:
    """Main TUI orchestrator for pythinker-psql."""

    PROMPT_SYMBOL_AI = "✨"
    PROMPT_SYMBOL_PSQL = "$"

    def __init__(self, soul: PythinkerSoul, psql_process: PsqlProcess):
        self.soul = soul
    ...
    def _create_prompt_session(self) -> PromptSession[str]:
        ...
        def get_prompt() -> FormattedText:
            ...
            return FormattedText([("bold fg:blue", f"pythinker-psql{symbol} ")])
    ...
    def _print_welcome(self) -> None:
        ...
        console.print(
            Panel(
                Text.from_markup(
                    "[bold]Welcome to pythinker-psql![/bold]\n"
    ...
    async def _run_ai_mode(self) -> None:
        ...
        # Run soul with visualize (same as pythinker-code shell)
        ...
        try:
            ...
            console.print("[red]LLM not set, run `pythinker /setup` to configure[/red]")
...
def main(
    ...
    """
    Start pythinker-psql: AI-assisted PostgreSQL interactive terminal.

    Usage is compatible with psql:
      pythinker-psql mydb
      pythinker-psql mydb postgres
      pythinker-psql -h localhost -U postgres -d mydb
      pythinker-psql --conninfo postgresql://user:pass@host/db
    """
...
async def _run_async(
    ...
    # Load config (same as pythinker-code)
    ...
    if not model:
        model = LLMModel(provider="pythinker", model="", max_context_size=250_000)
    if not provider:
    ...
    if not provider.base_url or not model.model:
        console.print("[red]LLM not configured. Run `pythinker /setup` to configure.[/red]")
        return
