#!/usr/bin/env python3
"""Clean up .pythinker sessions whose workdir is under a temporary directory.

This script handles two cases:
  1. Entries in pythinker.json whose path is a tmp directory -> remove entry + session dir.
  2. Orphan session directories on disk that have no matching pythinker.json entry
...
PYTHINKER_DIR = Path.home() / ".pythinker"
METADATA_FILE = PYTHINKER_DIR / "pythinker.json"
SESSIONS_DIR = PYTHINKER_DIR / "sessions"
...
def main() -> None:
    ...
    # --- Phase 1: tmp entries in pythinker.json ---
    ...
    for wd in work_dirs:
        if is_tmp_path(wd.get("path", "")):
            ...
            keep_hashes.add(work_dir_hash(wd["path"], wd.get("host", "local")))

    tmp_dirs: list[Path] = []
    for wd in tmp_entries:
        h = work_dir_hash(wd["path"], wd.get("host", "local"))
        session_dir = SESSIONS_DIR / h
        if session_dir.is_dir():
            tmp_dirs.append(session_dir)

    # --- Phase 2: orphan directories (on disk but not in pythinker.json) ---
