...
class JointSession(Session):
    """Combined session model with both web UI and pythinker-code session data."""

    model_config = ConfigDict(arbitrary_types_allowed=True)

    pythinker_code_session: PythinkerCLISession = Field(exclude=True)
...
def _build_pythinker_session(entry: SessionIndexEntry) -> PythinkerCLISession:
    from pythinker_host.path import HostPath

    return PythinkerCLISession(
        id=str(entry.session_id),
        work_dir=HostPath.unsafe_from_local_path(Path(entry.work_dir)),
...
def _build_joint_session(entry: SessionIndexEntry) -> JointSession:
    pythinker_session = _build_pythinker_session(entry)
    return JointSession(
        ...
        pythinker_code_session=pythinker_session,
