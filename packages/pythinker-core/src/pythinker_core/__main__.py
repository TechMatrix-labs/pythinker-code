...
async def main():
    ...
    parser.add_argument(
        "provider",
        choices=["pythinker", "openai", "anthropic", "google"],
    ...
    provider: Literal["pythinker", "openai", "anthropic", "google"] = args.provider
    ...
    match provider:
        case "pythinker":
            ...
            base_url = base_url or "https://api.pythinker-ai.ai/v1"
            assert api_key is not None, "Expect PYTHINKER_API_KEY environment variable"
            model = model or "pythinker-ai"

            chat_provider = Pythinker(base_url=base_url, api_key=api_key, model=model)
        case "openai":
