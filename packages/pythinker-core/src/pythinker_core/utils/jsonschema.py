...
def ensure_property_types(schema: JsonDict) -> JsonDict:
    ...
    The Pythinker AI (Pythinker) API rejects tool parameter schemas where a property
    ...
    but Pythinker AI's stricter validator returns HTTP 400 with
...
def _infer_type_from_values(values: list[JsonType]) -> str:
    ...
    - anything else mixed (e.g. ``[True, 1]`` or ``["a", 1]``) → fall back to
      ``"string"``, which Pythinker AI tolerates without cross-checking enum
