"""Tests for `_convert_tool` in the Pythinker chat provider.

These guard the Pythinker AI-specific schema quirk: Pythinker AI rejects tool
parameter schemas whose nested properties omit `type`, even though such
schemas are valid JSON Schema and accepted by OpenAI/Anthropic. The Pythinker
...
from pythinker_core.chat_provider.pythinker import (
    _convert_tool,  # pyright: ignore[reportPrivateUsage]
)
...
def _jetbrains_like_tool() -> Tool:
    ...
    the Pythinker AI 400 — `truncateMode` has `enum` but no `type`."""
    return Tool(
...
def test_convert_tool_preserves_builtin_function_shape() -> None:
    """Pythinker builtin functions (names starting with `$`) don't carry
