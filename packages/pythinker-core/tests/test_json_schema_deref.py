...
from pythinker_core.utils.jsonschema import deref_json_schema, ensure_property_types
...
def test_ensure_property_types_fills_missing_type_on_enum():
    """Regression for Pythinker AI 400: an MCP tool property with only `enum` and
    no `type` (as emitted by some JetBrains MCP tools, e.g. `truncateMode`)
    must have `type` filled in so the schema passes Pythinker AI validation."""
    schema: JsonSchema = {
        "type": "object",
        "properties": {
            "truncateMode": {
