...
1. Pythinker() constructed with api_key="token-A" → self._api_key = "token-A"
...
def test_on_retryable_error_preserves_refreshed_api_key():
    ...
    provider = Pythinker(model="test-model", api_key="token-A")
...
def test_api_key_correct_after_construction():
    ...
    provider = Pythinker(model="test-model", api_key="initial-token")
...
async def test_rebuilt_client_sends_correct_authorization_header():
    ...
    with respx.mock:
        respx.post("https://api.pythinker-ai.ai/v1/chat/completions").mock(
            side_effect=capture_headers
        )

        provider = Pythinker(model="test-model", api_key="token-A", stream=False)
