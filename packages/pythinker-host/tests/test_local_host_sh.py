"""Tests for `pythinker_host.exec` running commands via /bin/sh -c."""
...
async def run_sh(
    ...
    """Execute a shell command through pythinker_host.exec and collect exit code and streams."""
    process = await pythinker_host.exec("/bin/sh", "-c", command)
